...
### A9. PostHog Usage Telemetry (2026-06-29)
**Author:** Leela (Spec) + Squad (Implementation: Amy/.NET, Fry/Node.js, Hermes/Python) | **Owner:** Rido | **Status:** Implemented, awaiting review

Implement anonymous, opt-in PostHog usage telemetry for botas SDK to understand adoption and error patterns. Feature is disabled by default; enabled via `POSTHOG_API_KEY` environment variable. No PII collected.

**Specification (Leela):**
- 5 events: `botas/bot_started`, `botas/activity_received`, `botas/handler_dispatched`, `botas/handler_error`, `botas/outbound_sent`
- Distinct_id: SHA-256(CLIENT_ID)[:16] or "botas-anonymous" (identifies bot registration, not any person)
- Privacy: No message text, conversation IDs, user accounts, or IP forwarding (GeoIP disabled)
- Pipeline position: Fire-and-forget, non-blocking, errors swallowed; never affects turn latency
- Cross-language parity: Identical event schemas, property keys, and env var names across .NET/Node/Python

**Implementation Status:**
- **.NET** (Amy, `feat/dotnet-posthog`): `Botas/PostHogTelemetry.cs` (internal static), 9 tests, 178 total pass
  - Reflection-based SDK loading (optional NuGet dependency)
  - Lazy initialization, fire-and-forget via `Task.Run`
- **Node.js** (Fry, `feat/node-posthog`): `botas-core/src/posthog-telemetry.ts` (internal), 11 tests, 202 total pass
  - Dynamic import with type suppression, process exit handler for shutdown flush
- **Python** (Hermes, `feat/python-posthog`): `botas/_posthog_telemetry.py` (private), 18 tests, 224 total pass
  - Lazy import, atexit hook for shutdown flush
  - Fixed OTel dispatch_mode names (`typed`/`on_activity` → `type`/`catchall`)

**Configuration:**
- `POSTHOG_API_KEY` (required): PostHog project API key. Unset/empty disables telemetry.
- `POSTHOG_HOST` (optional): PostHog ingestion endpoint (default: `https://us.i.posthog.com`)

**Language-specific differences:** None. All three ports implement identical behavior, events, properties, and no-op pattern.

**Next steps:** Owner review + merge coordination across all three language branches.

### A10. PostHog Analytics for Documentation Site (2026-06-29)
**Author:** Kif (DevRel) | **Owner:** Rido | **Status:** Implemented, awaiting review

Integrate PostHog (`posthog-js`) into VitePress documentation site to track usage patterns (page views, navigation, search, clicks). Phase 1 of broader analytics effort; library instrumentation covered in A9.

**Implementation (Kif, `feat/telemetry-spec`):**
- **Package:** `posthog-js` (client-side JavaScript SDK)
- **Integration:** VitePress theme `enhanceApp` hook with router tracking
- **Configuration:** Environment variables (Vite-style)
  - `VITE_POSTHOG_KEY`: PostHog project API key. Unset disables tracking.
  - `VITE_POSTHOG_HOST`: PostHog instance URL (default: `https://us.i.posthog.com`)
- **Features:** Autocapture (clicks, page interactions), page tracking, custom `$pageview` events on route changes
- **Privacy/Security:** No PII, placeholder fallback prevents unintended data collection, SSR-safe (client-only init)
- **Documentation:** Updated `docs-site/observability.md` with setup instructions; `.env.example` template provided

**What PostHog tracks:** Page views, navigation flows, search queries, interaction events (link clicks, code samples)  
**What is NOT tracked:** User code, application telemetry (Phase 1 docs-site only), PII, sensitive bot credentials

**Alternatives considered (rejected):**
- Google Analytics: Heavier, more intrusive, less developer-focused
- Application Insights: Overkill for static docs, requires Azure subscription
- Plausible/Fathom: Less feature-complete than PostHog for product analytics
- No analytics: Need data-driven insights for docs prioritization

**Verification:**
- ✅ Build succeeds: `npm run docs:build` clean
- ✅ No TypeScript errors
- ✅ SSR-safe: guards for `typeof window` prevent server-side execution
- ✅ Security: no hardcoded keys, placeholder fallback non-functional
- ✅ Documentation: setup instructions in `observability.md`, `.env.example` created
...
- `docs-site/.gitignore`
- `docs-site/.env.example` (created)

**Next steps:** Owner review + production deployment when ready to enable data collection
