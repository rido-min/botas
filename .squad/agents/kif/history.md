...
- **2026-05-05**: **Reconciled A2 spec drift — ConversationClient public in .NET (PR #360).** PR #349 made `ConversationClient` public in .NET and exposed `CreateConversationAsync`, unlocking proactive messaging scenarios. The spec (`specs/proactive-messaging.md`) still claimed ".NET does NOT expose its ConversationClient publicly." **Solution**: (1) Removed outdated note that .NET doesn't expose ConversationClient; (2) Added .NET code examples for direct `ConversationClient.SendActivityAsync()` and `CreateConversationAsync()` usage alongside Node/Python examples; (3) Updated API feature table to show `.NET` now supports both core methods (SendActivityAsync and CreateConversationAsync) with checkmarks, while platform-specific methods (Update, Delete, GetMembers, etc.) remain Node/Python only; (4) Clarified that all three languages expose ConversationClient as a public property. **Pattern**: When implementation changes expose new public APIs, always grep specs/ and docs-site/ for stale visibility claims (e.g., "does NOT expose", "internal", "not available in"). **Files**: `specs/proactive-messaging.md` (lines 40–145). **PR**: #360, decision: A2 (Decision follow-up).
