...
def _get_distinct_id(client_id: Optional[str]) -> str:
    """Derive distinct_id from CLIENT_ID using SHA-256 (first 16 hex chars)."""
    if not client_id:
...
def track_event(event_name: str, properties: dict[str, Any], client_id: Optional[str] = None) -> None:
    ...
    Args:
        ...
        client_id: Bot application ID for deriving distinct_id.
    """
    ...
    with _lock:
        _initialize_client()
        if _is_disabled or _client is None:
            return

        # Derive distinct_id from client_id once
        if _distinct_id is None:
            _distinct_id = _get_distinct_id(client_id)
